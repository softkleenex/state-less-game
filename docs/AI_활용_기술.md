...
- 확정 동작을 순수 함수(`resolveLockPrecision`, `precisionMultiplierFor`)와 `requestAnimationFrame` 기반 마커 애니메이션으로 분리한 `SIGNAL LOCK` 타이밍 미터 구현, `prefers-reduced-motion`에서는 타이밍 단계를 건너뛰고 즉시 확정되도록 분기
- 라운드 종류와 무관하게 동일한 상단 지시문을 쓰도록 `ROUND_COPY`에서 `prompt`/`instruction`을 분리하고 공통 상수(`PLAY_INSTRUCTION`)로 통합
...
- `SIGNAL LOCK` 마커 위치가 타깃 중심·경계·바깥일 때 PERFECT/GOOD/MISS로 정확히 갈리는지, 정밀도 배수가 기존 점수 공식에 그대로 곱해지는지 테스트. 헤드리스 브라우저로 실제 클릭 두 번(고르기→확정)이 라운드를 정상 종료시키고, 라운드가 바뀌어도 상단 지시문이 동일하게 유지되며, `prefers-reduced-motion`에서 타이밍 단계가 생략되는지 점검
